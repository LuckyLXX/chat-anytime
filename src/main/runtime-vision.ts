...
export function buildVisionTools(deps: VisionToolDeps): ToolDefinition[] {
  return [
    defineTool({
      ...
      label: "识别图片",
      description: [
        "识别用户发送的图片或指定工作区图片文件，返回逐图描述，供你基于图片内容继续回答或操作。",
        "两类用法：用户本轮附带图片而当前对话模型不支持图片输入——先调用本工具识别再回答；或查看工作区图片文件（如你自己生成的截图），传 files（相对路径，最多 5 个，结果标注文件名）。",
        "prompt 可选：按当前需要指定重点识别什么、按什么格式描述（如逐字转写报错对话框、按序列出图表数据点）；不传则默认全面描述。识别失败按错误信息修正后重试。"
      ].join(""),
