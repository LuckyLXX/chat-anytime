...
export function App(): ReactNode {
  ...
  /** 无已存状态时的默认值按目标类型分派：markdown 文件默认编辑态，记忆主题默认预览态（点开先读，铅笔进入编辑）。 */
  function getEditorState(tabId: string, target?: PreviewTarget): PreviewEditorState {
    const state = previewEditorStates[tabId];
    if (state) return state;
    return target?.type === "memory" ? { ...defaultEditorState(), editing: false } : defaultEditorState();
  }
